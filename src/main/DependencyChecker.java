...
public class DependencyChecker {
    public static boolean isPythonAvailable() {
        ...
        try {
            ...
            // If the exit code is 0 and the output contains "Python 3", it means Python 3 is available
            if (exitCode == 0 && line != null && line.contains("Python 3")) {
                isAvailable = true;
            }

        } catch (IOException | InterruptedException e) {
            // Handle exceptions if the command couldn't be executed or if waiting for the process fails
            e.printStackTrace();
            return false;
        }
        return isAvailable;
    ...
    public static boolean isNbtlibPackageAvailable() {
        ...
        try {
            ...
            // If the exit code is 0 and the package information is found, it means nbtlib is available
    ...
    public static void checkDependencies() {
        ...
        if (pythonAvailable) {
            ...
            JOptionPane.showMessageDialog(null, "Python 3 is required to run this application. Please install Python 3 and try again.", "Dependency Error", JOptionPane.ERROR_MESSAGE);
            System.exit(1);
        }
        ...
        if (!nbtlibAvailable && !nbtlib3Available) {
            JOptionPane.showMessageDialog(null, "The nbtlib package is required to run this application. Please install nbtlib and try again.", "Dependency Error", JOptionPane.ERROR_MESSAGE);
            System.exit(1);
        }
    }
